...
//Return all jobs which are not yet expired
...
export const postJob = catchAsyncErrors(async (req, res, next) => {
  ...
  if ((!salaryFrom || !salaryTo) && !fixedSalary) {
    return next(
      new ErrorHandler(
        "Please  provide either fixed salary or range of salary.",
        400
      )
    );
  }
  ...
  if (salaryFrom && salaryTo && fixedSalary) {
    return next(
      new ErrorHandler("Unable To Enter Fixed and Ranged Salary together.", 400)
    );
  }
